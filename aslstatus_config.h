...
static const char IFC[] = "wlp2s0";  /* wifi interface */

/* temporarily to get sound percentage for pulseaudio */
/* static const char pulse_volume[] = "if [ $(pulsemixer --get-mute) -eq 0 ];then echo $(pulsemixer --get-volume | awk  '{print $1}');else echo muted;fi"; */
...
static const char pulse_volume[] = "pactl subscribe | "
	...
	" xargs kill -9)";
static const char check_updates[] ="if [ $(/usr/bin/checkupdates | wc -l) -eq 0 ];then echo ' ';else echo '  ';fi";
...
{ run_command,		"[%s]",	"curl -fsSL https://wttr.in/istanbul?format=3",	 30 MIN,	END }, 
{ run_command,		"[%s]",	"if  pgrep  spotifyd >/dev/null ;then echo $(spt pb -s);else echo 'off';fi",	 1 SEC,	END }, 
{ vol_perc,		"[墳 %s]",	NULL,		 0,	END },*/

{ run_command, 		"%s", 		check_updates,	 30 MIN, END},
{ run_command,		"[墳 %s]",	pulse_volume,	 1,	END }, 
{ cpu_perc,		"[ %s%%]",	NULL,		 1 SEC,	END },
{ ram_used,		"-[﬙ %sB]", 	NULL,		 3 SEC,	END },


{run_command, "-[VPN  %s]","cat /tmp/isvpnon" ,1 SEC,END},
{ netspeed_rx, "-[ %sB/s |","wlp2s0",1 SEC,END},
{ netspeed_tx, " %sB/s ]","wlp2s0",1 SEC,END },

{ battery_state,	"-[%s",		"BAT0",		 2 SEC,	END },
{ battery_perc,		" %s%%]",	"BAT0",		 1 MIN,	END },
{ datetime, "%s",   " %a %d %b %r",          1 SEC ,END },
